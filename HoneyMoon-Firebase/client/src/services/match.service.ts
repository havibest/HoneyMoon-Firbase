...
export async function recordPass(fromUid: string, toUid: string): Promise<void> {
...
export async function getSeenProfiles(uid: string): Promise<Set<string>> {
  const [likesSnap, passesSnap] = await Promise.all([
    getDocs(query(collection(db, "likes"), where("fromUid", "==", uid))),
    getDocs(query(collection(db, "passes"), where("fromUid", "==", uid))),
  ]);

  const seen = new Set<string>();
  likesSnap.docs.forEach((d) => seen.add(d.data().toUid));
  passesSnap.docs.forEach((d) => seen.add(d.data().toUid));
  return seen;
}
...
export async function getDiscoverQueue(
  ...
  const seen = await getSeenProfiles(uid);
  ...
  const realProfiles: DiscoverProfile[] = snap.docs
    ...
    .filter((p) => p.id !== uid && !seen.has(p.id));
