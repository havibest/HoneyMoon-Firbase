...
import { selectReferralPlan } from "@/services/subscription.service";
...
export default function SubscriptionPage() {
  ...
  const handleSelectPlan = async (referrals: 0 | 1 | 2 | 5, price: number) => {
    try {
      setLoading(referrals);
      await selectReferralPlan(profile.uid, referrals, price);
      await refreshProfile();

      if (referrals === 0) {
        // Pay now — go to checkout
        navigate("/checkout");
      } else {
        // Referral plan — go to referrals page
        toast.success(`Plan selected! You need ${referrals} paying referral${referrals > 1 ? "s" : ""} to unlock access.`);
        navigate("/referrals");
      }
    } catch (error) {
              ...
              >
                ...
                <CardContent className={`p-6 ${isPopular ? PLAN_BG[i] : ""}`}>
                  ...
                  <Button
                    ...
                    onClick={() => handleSelectPlan(plan.referrals as 0 | 1 | 2 | 5, plan.price)}
                  >
