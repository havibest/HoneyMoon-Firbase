...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    Promise.all(Array.from(new Set(missing)).map((uid) => getProfile(uid))).then((profiles) => {
      setParticipants((prev) => {
        const next = { ...prev };
        profiles.forEach((p) => { next[p.uid] = p; });
        return next;
      });
    });
  ...
  const handleSend = async () => {
    ...
    setText("");
    try {
      await sendMessage(activeId, user.uid, value);
    } catch {
      setText(value);
    }
  };
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        <p className="truncate text-sm font-semibold text-foreground">{other?.displayName || "Member"}</p>
                        {unread > 0 && (
            ...
            <>
              <div className="flex items-center gap-3 border-b border-border p-3">
                ...
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-foreground">{activeOther?.displayName || "Member"}</p>
                  {activeOther?.online && <p className="text-xs text-green-600">Online</p>}
                </div>
              </div>
